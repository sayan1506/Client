/* ============================================================
   GhostCursor — vanilla port of the React Bits component
   Three.js smoky cursor trail with bloom + film grain.
   createGhostCursor(hostEl, options) -> { dispose }
   ============================================================ */
...
export function createGhostCursor(host, options = {}) {
  ...
  let trailBuf = Array.from({ length: maxTrail }, () => new THREE.Vector2(0.5, 0.5));
  let head = 0;
  ...
  function animate() {
    if (!active) return;
    if (!hasValidSize) { raf = requestAnimationFrame(animate); return; }
    ...
    if (pointerActive) {
      ...
      if (dt > fadeDelay) fadeOpacity = Math.max(0, 1 - Math.min(1, (dt - fadeDelay) / fadeDuration));
    }
    ...
    if (!pointerActive && fadeOpacity <= 0.001) { running = false; raf = null; return; }
